...
class ConnectController extends Controller
{
    ...
    public function postLogin(Request $request){
        ...
        $messages  = [
            ...
            'email.email' => 'EI formato de su correo electrónico es invalido',
    ...
    public function postRegister(Request $request){
        ...
        $messages = [
            ...
            'cpassword.min' => 'La confirmación de la contraseña de tener al menos 8 caracteres',
    ...
    public function getLogout(){
        ...
        Auth::logout();    
        if($status == "100"):
            return redirect('/login')->with('message', 'Su usuario esta SUSPENDIDO.')-> with('typealert', 'danger');
        else:
            return redirect('/');
        endif;
    ...
    public function postRecover(Request $request){
        ...
        if($validator->fails()):
            ...
            if($user == 1):
                ...
                return redirect('/reset?email='.$user->email)->with('message', 'Ingrese el codigo que le hemos enciado a su correo electrónico.')-> with('typealert', 'success');
                endif; 
            else:
                return back()->with('message', 'Este Correo Electrónico no Exixte')-> with('typealert', 'danger');
            endif;
        endif;
    }
        ...
        if($validator->fails()):
            ...
            if($user == 1): 
                ...
                if($user->save()):
                    ...
                    return redirect('/login')->with('message', 'La contraseña fue restablecida con exito, le hemos enviado un correo electrónico con su nueva contraseña para que pueda iniciar sesión.')-> with('typealert', 'success');
                endif;

            else:
                return back()->with('message', 'El correo electrónico o el codigo de recuperacion son erroneos')-> with('typealert', 'danger');
            endif;
        endif;
    }
}